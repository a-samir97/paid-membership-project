...
stripe.api_key = 'sk_test_3DlMDtqBHorNgpTsVrBnQ4T900FkYp54GI'
...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        # monthly plan id
        plan = 'plan_H4eUWjAFHSidCk'
        # check if the plan is yearly not monthly
        if request.POST['plan'] == 'yearly':
            # yearly plan id
            plan = 'plan_H4eV5eHzvdWPdV'
        if request.POST['coupon'] in coupons:
            ...
            try:
                ...
                pass
            subscription = stripe.Subscription.create(customer=stripe_customer,
                                                      items=[{'plan':plan}],
        ...
        return render(request, 'plans/checkout.html', {'coupon':coupon,'plan':plan,
                                                       'og_dollar':og_dollar,'coupon_dollar':coupon_dollar,
                                                       'final_dollar':final_dollar,'price':price})
